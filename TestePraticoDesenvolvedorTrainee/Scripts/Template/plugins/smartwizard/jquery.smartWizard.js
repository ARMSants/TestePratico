...
;(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        toolbarSettings: {
            ...
            finishFunction: null // Funtion to execute on click button
        },
        anchorSettings: {
    ...
    $.extend(SmartWizard.prototype, {
        ...
        _setToolbar: function () {
            ...
            var btnNext = this.options.toolbarSettings.showNextButton !== false ? $('<button></button>').text(this.options.lang.next).addClass('btn btn-secondary sw-btn-next btn-info').attr('type', 'button') : null;
            var btnPrevious = this.options.toolbarSettings.showPreviousButton !== false ? $('<button></button>').text(this.options.lang.previous).addClass('btn btn-secondary sw-btn-prev btn-default').attr('type', 'button') : null;
            var btnFinish = this.options.toolbarSettings.showPreviousButton !== false ? $('<button></button>').text(this.options.lang.finish).addClass('btn btn-secondary sw-btn-finish btn-success').attr('onclick',this.options.toolbarSettings.finishFunction).attr('type', 'button') : null;
            var btnGroup = $('<div></div>').addClass('btn-group mr-2 sw-btn-group pull-right').attr('role', 'group').append(btnPrevious, btnNext,btnFinish);

            // Add extra toolbar buttons
            var btnGroupExtra = null;

            if (this.options.toolbarSettings.toolbarExtraButtons && this.options.toolbarSettings.toolbarExtraButtons.length > 0) {
                btnGroupExtra = $('<div></div>').addClass('btn-group mr-2 sw-btn-group-extra pull-right').attr('role', 'group');
        ...
        _setEvents: function () {
            ...
            // Finish button event
            $('.sw-btn-finish', this.main).on("click", function (e) {
                e.preventDefault();
                mi._transitPage(0);
            });
        ...
        _setAnchor: function (idx) {
            ...
            this.steps.eq(idx).parent('li').addClass("active").nextAll('li').removeClass("done");;
        ...
        _setButtons: function (idx) {
            ...
            if (!this.options.cycleSteps) {
                if (0 >= idx) {
                    $('.sw-btn-prev', this.main).addClass("hide");
                } else {
                    $('.sw-btn-prev', this.main).removeClass("hide");
                }
                if (this.steps.length - 1 <= idx) {
                    $('.sw-btn-next', this.main).addClass("hide");
                    $('.sw-btn-finish', this.main).removeClass("hide");
                } else {
                    $('.sw-btn-next', this.main).removeClass("hide");
                    $('.sw-btn-finish', this.main).addClass("hide");
                }
            }
            return true;
        },
